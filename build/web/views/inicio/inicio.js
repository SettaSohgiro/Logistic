  ...
  function apiRest(){   
      var rest = [];
        ...
        .then(function (response) {
          ...
          var dolar = response.data.dolar.valor;
          var fecha = response.data.dolar.fecha.substr(0 , 10); 
          var fecha = fecha.substr(8,2)+"/"+fecha.substr(5 , 2 )+"/"+fecha.substr(0,4);
          $("#ston_dollar").text("");
          $("#ston_dollar").html(dolar);
          console.log(fecha);
                          $.ajax({
                    type: "post",
                    url: "inicio",
    ...
    var webAudienceMetricsSatackedData = {
      ...
      datasets: [
        {
          label: 'Sesiones',
          data: [4000,2300,4000,6000,2000,2000,2000,4000,8000],
          backgroundColor: [
            '#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc',
          ],
          borderColor: [
            '#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc','#3794fc',
          ],
      ...
      },
      {
        ...
        backgroundColor: [
          '#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef',
        ],
        borderColor: [
          '#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef','#dee5ef',
        ],
    ...
    var webAudienceMetricsSatackedOptions = {
      ...
      legendCallback: function(chart) {	
        ...
        for (var i = 0; i < chart.data.datasets.length; i++) {
          text.push('<div class="col-lg-4"><div class="row"><div class="col-sm-12"><h5 class="font-weight-bold text-dark mb-1">' + chart.data.datasets[i].data[8].toLocaleString() + '</h5></div></div><div class="row align-items-center"><div class="col-2"><span class="legend-label" style="background-color:' + chart.data.datasets[i].backgroundColor[i] + '"></span></div><div class="col-9 pl-0"><p class="text-muted m-0 ml-1">' + chart.data.datasets[i].label + '</p></div></div>');
          text.push('</div>');
        }
        text.push('</div>');
        return text.join("");
