...
//                {mData: "Borrar"},
    ...
    //CODIGO QUE COLOCA LOS REGISTROS EN LA TABLA
    var tableCV = "";
    function tableContratosVenta() {
        $('#contratos_table').dataTable().fnClearTable();
        $('#contratos_table').dataTable().fnDestroy();


        tableCV = $('#contratos_table').dataTable({
            ...
            //"sAjaxDataProp": "demo",
            "ajax": {
                url: "contratosventaservlet", // json datasource
                type: "post", // method  , by default get
                data: {"func": "tablaContratosVenta",
                    "type": 0},
            ...
            "aoColumns": [
                ...
                {mData: "FechaInicio"},
                {mData: "FechaTermino"},
                {mData: "NombreContrato"},
                {mData: "Estado"},
                {mData: "Cliente"},
                {mData: "Solicitud"},
                {mData: "Transporte"},
                {mData: "ResumenVenta"},
                {mData: "ActualizarContrato"},
//                {mData: "Borrar"},
            ],
            initComplete: function () {
                // Apply the search
...
//                    var resp = JSON.parse(data);
        ...
        } else if (tipo == "publicar_contrato") {
            $("#modalmsnsn").addClass("show").css("display", "block").removeAttr("aria-hidden").attr("aria-modal", "true");
            $("#mdlbmsnsn").text("");
            $("#mdlbmsnsn").html("¿Esta seguro que desea publicar esta solicitud?");
            ...
            $("#btn_mod_si").on("click", function () {
                ...
                $.ajax({
                    type: "post",
                    url: "contratosventaservlet",
                    data: {"func": "crearContratoVenta",
                        "fechaInicio": date,
                        "id_solicitud": datasolicitud["ID"],
                        "id_cliente": 0},
